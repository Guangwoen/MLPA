...
namespace mlpa::reg {
LASSORegression::LASSORegression(Eigen::RowVectorXd X, Eigen::VectorXd y, const unsigned k=1)
    : RegressionBase(std::move(X), std::move(y), k), m_nWSR(100), m_lambda(0) {}

LASSORegression::LASSORegression(Eigen::RowVectorXd X, Eigen::VectorXd y, const double l, const unsigned k=1)
    : RegressionBase(std::move(X), std::move(y), k), m_nWSR(100), m_lambda(l) {}

LASSORegression::LASSORegression(Eigen::RowVectorXd X, Eigen::VectorXd y,
    const double l, const int w=100, const unsigned k=1)
    : RegressionBase(std::move(X), std::move(y), k), m_nWSR(w), m_lambda(l) {}

void LASSORegression::estimate() {
    const auto Phi = m_phi(m_X);

    const auto multPhi = Phi * Phi.transpose();
    ...
    const auto var_size = static_cast<qpOASES::int_t>((m_k+1) * 2);
...
Eigen::VectorXd LASSORegression::predict(const Eigen::RowVectorXd& X_star) {
