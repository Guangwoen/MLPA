...
def colorsegms(segm, img):
    ...
    for i in range(1, np.int64(max(segm.flatten())) + 1):
...
def labels2seg(Y,L):
    ...
    rstep = np.int64(np.floor(L['stepsize']/2.0))
    stepbox = range(-rstep, L['stepsize'] - rstep)
    rx = np.asarray(L['rangex'], dtype=np.int64) + np.int64(L['offset'])
    ry = np.asarray(L['rangey'], dtype=np.int64) + np.int64(L['offset'])
